...
{
    const auto length = static_cast<size_t>(inAddr->iSockaddrLength) <= c_saddrSize ? inAddr->iSockaddrLength : c_saddrSize;
...
{
    const auto length =
        static_cast<size_t>(inAddr->iSockaddrLength) <= c_saddrSize ? static_cast<size_t>(inAddr->iSockaddrLength) : c_saddrSize;
...
inline bool ctSockaddr::WriteCompleteAddress(WCHAR (&address)[c_ipStringMaxLength], bool trimScope) const noexcept
{
    ...
    if (0 == WSAAddressToStringW(sockaddr(), static_cast<DWORD>(c_saddrSize), nullptr, address, &addressLength))
    {
        if (family() == AF_INET6 && trimScope)
        {
            ...
            auto* pScopePtr = std::find(address, end, L'%');
            if (pScopePtr != end)
            {
                const WCHAR* pMovePtr = std::find(address, end, L']');
                if (pMovePtr != end)
                {
                    while (pMovePtr != end)
                    {
...
{
    return const_cast<SOCKADDR_INET*>(reinterpret_cast<const SOCKADDR_INET*>(&m_saddr));
}
