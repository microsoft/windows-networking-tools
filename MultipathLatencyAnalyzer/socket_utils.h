...
{
    constexpr DWORD flags = WSA_FLAG_OVERLAPPED;
...
inline void SetSocketOutgoingInterface(SOCKET socket, short family, int outgoingIfIndex)
{
    ...
    if (family == AF_INET)
    {
        ...
        constexpr auto length = sizeof(value);
    ...
    else if (family == AF_INET6)
    {
        ...
        constexpr auto length = sizeof(outgoingIfIndex);
        const auto error =
...
inline void SetSocketReceiveBufferSize(SOCKET socket, int size)
{
    ...
    constexpr auto optionLength = sizeof(optionValue);
